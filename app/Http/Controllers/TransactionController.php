...
class TransactionController extends Controller
{
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        if ($transaction->amount > 700) {
            SecurityLog::create([
                'message' => "Transaction ID: {$transaction->id} exceeded the limit of $700.",
                'status' => 'pending',
                'type' => 'high_value_transaction',
                'description' => "Transaction ID {$transaction->id} has exceeded the $700 threshold. Please review the transaction details.", // Add a description here
            ]);
        }
